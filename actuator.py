...
class Actuator:
    ...
    def recv_can_answ(self):
        ...
        new_pos = (res_ar[6] + (res_ar[7] << 8) - self.pos_offset)*self.pos_scale
        ...
        vel = (new_pos + self.rev_n*2*np.pi - self.state.X[0])/(time.time() - self.state.time)
    ...
    def send_current(self, current):
        ...
        msg = can.Message(arbitration_id = self.id,
                      data=[0xA1, 0, 0, 0, current & 0x00FF, current >> 8, 0, 0],
